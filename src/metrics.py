...
def compute_metric(task_name, args, generation_pipeline):
    _metrics = {
        ...
        "answer-relavancy": metrics.AnswerRelevancyMetric(
            threshold=0.7, model=args.judge
        ),
    }
...
def generate_results(args, generation_pipeline, dataset, task_name):
    ...
    for entry in tqdm(dataset, desc="Generating responses...", unit="query"):
        query = helper.get_metric_prompt(
            task_name, entry["query"], entry.get("context")
        )
...
def compute_score(args, results: list, metric, task_name: str):
    ...
    for i, entry in enumerate(results):
        test_case = LLMTestCase(
            ...
            retrieval_context=entry.get("context"),
            context=entry.get("context"),
        )
    ...
    if args.save_results:
        helper.save_json(
            measurement_results, config.RESULTS_DIR, f"{task_name}-eval-results.json"
        )
    return final_score
...
def evaluate_toxicity_with_bert(args, results) -> None:
    pipe = pipeline(
        "text-classification", model="RabidUmarell/toxic-hubert", device=args.device
    )
    ...
    if args.save_results:
        helper.save_json(
            bert_results, config.RESULTS_DIR, "toxicity-bert-eval-results.json"
        )
