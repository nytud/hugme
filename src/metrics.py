...
def compute_metric(task_name, args, generation_pipeline):
    _metrics = {
        "bias": metrics.BiasMetric(threshold=THRESHOLD, model=args.judge),
        "hallucination": metrics.HallucinationMetric(threshold=THRESHOLD, model=args.judge),
        "answer-relavancy": metrics.AnswerRelevancyMetric(threshold=THRESHOLD, model=args.judge),
    }
